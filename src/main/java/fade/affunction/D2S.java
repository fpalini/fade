...
public class D2S extends AFFunctionEvaluatorByStatistic implements Similarity {
    public D2S(Configuration conf) {
        super(conf);
    }
    ...
    public AFValue evaluatePartialAFValue(Value s1, Value s2) {
        ...
        return new AFValue((count1_mod * count2_mod) / Math.sqrt(Math.pow(count1_mod, 2) + Math.pow(count2_mod, 2)));
    }
