...
public class Intersection extends AFFunctionEvaluatorByStatistic implements Similarity {
    public Intersection(Configuration conf) {
        super(conf);
    }
    ...
    public AFValue evaluatePartialAFValue(Value s1, Value s2) {
        ...
        return new AFValue(Math.min(count1, count2) / (count1 + count2));
    }
