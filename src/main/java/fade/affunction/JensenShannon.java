...
public class JensenShannon extends AFFunctionEvaluatorByStatistic {
    public JensenShannon(Configuration conf) {
        super(conf);
    }
    ...
    public AFValue evaluatePartialAFValue(Value s1, Value s2) {
        ...
        double freq_m = (freq1 + freq2) / 2;

        return new AFValue(freq1 * Math.log(freq1) + freq2 * Math.log(freq2) - 2 * freq_m * Math.log(freq_m));
    }
